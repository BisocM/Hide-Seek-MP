...
function hudShowBanner(text, color, textColor)
	_hud.bannerQueue[#_hud.bannerQueue + 1] = { text = text, color = color, textColor = textColor, time = 0.0 }
end
...
function hudDrawGameSetup(settings)
	...
	if not IsPlayerHost(GetLocalPlayer()) then
			...
			for i = 1, #settings do
				local group = settings[i]
					for j = 1, #group.items do
						...
						if #options > 0 then
							local hasKey = type(HasKey) == "function" and HasKey(item.key) or false
							if hasKey and not _hud.settings.initiated then
								local index = GetInt(item.key .. ".index")
								if index < 1 or index > #options then
									index = 1
								end
								SetInt(item.key .. ".index", index)
								SetString(item.key, options[index].value)
							elseif not hasKey then
								SetInt(item.key .. ".index", 1)
								SetString(item.key, options[1].value)
							end
						end
					end
				end
		...
		uiDrawPanel(width, height, 16)
			...
			UiColor(COLOR_WHITE)
			UiFont(FONT_BOLD, FONT_SIZE_30)
			UiText(hsT("hs.ui.hostMenu.title"))
			UiTranslate(0, 26 + 10)
			
			if hasSettings then
				if uiDrawSecondaryButton(hsT("hs.ui.hostMenu.settings"), 290) then
		...
		if hasSettings and _hud.settings.animation > 0 then
			UiTranslate(-750*(1.0-_hud.settings.animation), 0)
...
function _drawSettings(settings)
	...
	UiColor(COLOR_WHITE)
	UiText(hsT("hs.ui.settings.title"))
	UiTranslate(0, 36)
	...
	if uiDrawSecondaryButton(hsT("hs.ui.settings.resetDefaults"), 210) then
		for i = 1, #settings do
			local group = settings[i]
			for j = 1, #group.items do
				local item = group.items[j]
	...
	UiPush()
	UiTranslate(105 + 10,0)
	if uiDrawSecondaryButton(hsT("hs.ui.settings.close"), 210) then
		SetValueInTable(_hud.settings, "animation", 0, "easeout", 0.4)
...
function _drawSettingsItemStepper(key, title, info, options, width, cyclic)
	...
	UiPush()
	UiColor(COLOR_WHITE)
	uiDrawTextEllipsis(title, "regular.ttf", 23*1.23, math.max(1, width - 242 - 16), 1)
	UiPop()
...
function hudDrawBanner(dt)
	...
	UiPush()
		...
		UiPop()
		if type(banner.textColor) == "table" then
			UiColor(banner.textColor[1] or 1, banner.textColor[2] or 1, banner.textColor[3] or 1, banner.textColor[4] or 1)
		else
			UiColor(COLOR_WHITE)
		end
		UiColorFilter(1, 1, 1, textAlpha)
